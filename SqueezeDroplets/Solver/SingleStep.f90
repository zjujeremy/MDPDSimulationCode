...
!   Locals
    ...
    integer k, isample, stepUnEquil, n
    real    Sampletime

    stepCount = stepCount + 1
    
     !move the up wall to down by small velocity  2017.8.23 by lijm
    if(stepCount .ge. startSqueeze .and. stepCount .lt. (stepLimit - delta_step)) then
        do n = 1 , nWallAtom
            if(wr_mark(n) .eq. 1) then
                r(n,3) = r(n,3) - wall_velo
            endif
        enddo
    endif
