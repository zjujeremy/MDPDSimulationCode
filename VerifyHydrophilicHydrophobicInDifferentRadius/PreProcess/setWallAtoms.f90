...
!-------------------------------------------------------------------------
    ...
    subroutine setWallAtoms(fgap, wgap, n, h3,widtotx,solidfractionx,widtoty,solidfractiony,ntextlay,keytext)
...
!   Parameters
    ...
    integer	n
    real*8	wgap, fgap, h3,h4,ht1

!   Locals

    integer	MAXCTP, MAXWATM, MAXATOM, NDIM
    parameter (MAXCTP = 10, MAXWATM = 10000000, MAXATOM = 30000000, NDIM = 3)
    integer	i, j, k, l, n1, n2, ny, nctp, nline, nAtom, nWallAtom, nsubdom,textz,ntextlay,keytext
    ...
    real*8	wx1(4,MAXCTP), wx2(4,MAXCTP), wz1(4,MAXCTP), wz2(4,MAXCTP), &
            wx3(4,MAXCTP), wz3(4,MAXCTP),wx4(4,MAXCTP), wz4(4,MAXCTP), norml(2,MAXCTP,2),&
            wxt1(4,MAXCTP), wzt1(4,MAXCTP),tempgap1,tempgap2,widtotx,solidfractionx,widtoty,solidfractiony
...
!   h1 =  0.25*fgap
    ...
    do i = 1, nline
        ...
        do j = 2, nctp - 1
            ...
            wx4(i,j) = (-(a1 + h4)*tx2 + (a2 + h4)*tx1)/delta
            wz4(i,j) = ( (a2 + h4)*tz1 - (a1 + h4)*tz2)/delta
            wxt1(i,j) = (-(a1 + ht1)*tx2 + (a2 + ht1)*tx1)/delta
            wzt1(i,j) = ( (a2 + ht1)*tz1 - (a1 + ht1)*tz2)/delta            
        enddo
        ...
        wx4(i,nctp) = x2 + h4*tz2
        wz4(i,nctp) = z2 - h4*tx2
        wxt1(i,nctp) = x2 + ht1*tz2
        wzt1(i,nctp) = z2 - ht1*tx2        
    enddo
...
!                    endif
!                    if(k .le. n1) then
!                        if(nctp .eq. 2 .and. k .eq. n1) goto 35
!                        n = n + 1
!                        rw(n,1) = c(1) + 0.5*gap1*tx1
!                        rw(n,2) = c(2) + 0.5*gap2
!                        rw(n,3) = c(3) + 0.5*gap1*tz1
    ...
    if(keytext .eq. 1) then
        do textz = 1 , ntextlay*2
            ...
            do i = 1, nline
                ...
                do j = 1, nctp - 1
                    x1 = wxt1(i,j)
                    z1 = wzt1(i,j) + 0.5*wgap*d*(textz - 1)
                    x2 = wxt1(i,j+1)
                    z2 = wzt1(i,j+1) + 0.5*wgap*d*(textz - 1)
                    length = sqrt((x2 - x1)**2 + (z2 - z1)**2)
                    n1 = int(length/wgap)
        !           if(n1*wgap .lt. length) n1 = n1 + 1
                    if(j .eq. 1) then
                    ...
                    do k = 1, n2
                        ...
                        tempgap1 =int((c(1) - x1 - (1 - 0.25)*gap1*tx1)/widtotx/gap1)
                        tempgap1 = tempgap1 * widtotx * gap1 - length/2
                        tempgap2 = widtotx*solidfractionx*gap1
                        if((c(1)-tempgap1) .le. tempgap2)then
                            do l = 1, ny
                                c(2) = (l - 0.75)*gap2 - regionH(2)
                                tempgap1 =int((c(2) - (1 - 0.75)*gap2 + regionH(2))/widtoty/gap2)
                                tempgap1 = tempgap1 * widtoty * gap2 - regionH(2)
                                tempgap2 = widtoty*solidfractiony*gap2
                                if((c(2)-tempgap1) .le. tempgap2)then
                                    if(j .eq. 1 .or. k .ne. 1) then
                                        n = n + 1
                                        rw(n,1) = c(1)
                                        rw(n,2) = c(2)
                                    ...
                                    if(k .le. n1) then
                                        if(nctp .eq. 2 .and. k .eq. n1) goto 45
                                        n = n + 1
                                        rw(n,1) = c(1) + 0.5*gap1*tx1
                                        rw(n,2) = c(2) + 0.5*gap2
                                        rw(n,3) = c(3) + 0.5*gap1*tz1
