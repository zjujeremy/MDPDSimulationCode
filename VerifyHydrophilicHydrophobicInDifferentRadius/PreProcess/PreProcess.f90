...
!-------------------------------------------------------------------
    ...
    parameter (MAXCTP = 10, MAXWATM = 10000000, MAXATOM = 30000000, NDIM = 3, &
               MAXBDS = 500000, MAXDP = 30)
    integer	i, j, k, n, nline, ly, nAtom, nWallAtom, nsubdom, &
            nChain, ChainLen, nChainend, nDpEnd, seeds, nBeads1, nBeads2, nDp,ntextlay
    integer	xuc(4,MAXCTP), zuc(4,MAXCTP), ncontrlp(MAXCTP),   &
            initUcell(NDIM), key2, DpFzn(MAXDP),ntemp(2),nPDP,LenDp,keytext
    integer	isbm(MAXCTP,8), nchsb(MAXCTP), ndpsb(MAXCTP), ncc(MAXBDS,NDIM)
    real*8	density,dratio, ratio, fgap, wgap, wmingap, h3, wLayer, RdsDp,lenDpL
    real*8	r(MAXATOM,NDIM), wn(MAXWATM,NDIM), regionH(NDIM), &
            region(NDIM), gap(NDIM),tempr(MAXBDS,NDIM),tChainLen
    real*8	subdom(MAXCTP,8), Cx(MAXDP), Cy(MAXDP), Cz(MAXDP),widtotx,solidfractionx,widtoty,solidfractiony
    character key1, lattice*3
    character*30 fdata
    integer drop_particle_num, small_tube_slices_num, big_tube_slices_num
    ...
    common /Pdpr/ nPDP
    common /HysteresisAngles/ drop_particle_num, small_tube_slices_num, big_tube_slices_num
    ...
    open(10, file = './Paras_Config/lint.dat')
    if(key1 .eq. 'y' .or. key1 .eq. 'Y') then
        ...
        write(*,*) 'including textured surfaces or not?(1/0)'
        read (*, *) keytext
        if(keytext .eq. 1) then
            write(* ,*) 'enter parameters of textured surfaces:widtotx,solidfractionx,widtoty,solidfractiony,ntextlay'
            read(10,*) widtotx,solidfractionx,widtoty,solidfractiony,ntextlay
        endif
    else
        ...
        read(10,*) LenDp, dratio
        read(10,*) key2
        if(key2 .eq. 1) then
            read(10,*) RdsDp, nDp,dratio
            do i = 1, nDp
                read(10,*) Cx(i), Cy(i), Cz(i)
                Cz(i) = ((4.0 / density)**(1./3.))*zuc(1,1)+RdsDp
            enddo
        else
            nDp   = 0
            RdsDp = 0.0
...
!            call setWallAtoms(fgap, wgap, nWallAtom, h3)
            ...
            do n=nWallAtom+1,nAtom      
                if(abs(r(n,3)) .gt. (regionH(3)-lenDpL))then
                    i=i+1
                    do k=1,NDIM                    
                        tempr(i,k) = r(n,k) 
                    enddo
 ...
 !   endif
    ...
    enddo
    do n = 1, nWallAtom
	   write(16,*) (wn(n,k), k = 1, NDIM)
    enddo
    if(nDp .gt. 0) then
        do n = 1, nBeads1
            write(16,*) (ncc(n,k), k = 1, NDIM)
        enddo
    endif
    if(nChain .gt. 0) then
        do n = nBeads1+1, nBeads1+nBeads2
            write(16,*) (ncc(n,k), k = 1, NDIM)
        enddo
    endif
...
!	Locals
	...
	parameter (MAXWATM = 10000000, MAXATOM = 30000000, NDIM = 3)
