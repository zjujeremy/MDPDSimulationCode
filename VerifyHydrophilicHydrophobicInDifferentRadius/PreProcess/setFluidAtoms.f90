...
!   Parameters
    ...
    real*8	fgap, wLayer, RdsDp, Hy_slice_high
...
!   integer lDP, lF                                 !(Irving-Kirkwood interfacial tension test)
    ...
    do idom = 1, nsubdom
        ...
        enddo
        ! 添加测量前进角和后退角的针管滴加液体的前处理程序  add by lijm 2017.10.23
        !drop_particle_num = m1
        !small_tube_slices_num = 72
        !big_tube_slices_num = 40
        !do k = 1, small_tube_slices_num
        !    Hy_slice_high  = -7 + (k-1)*fgap/4
        !    do j = -4, 4, 1
        !        do i = -4, 4, 1
        !            m1 = m1 + 1
        !            rb(m1,3) = Hy_slice_high
        !            rb(m1,2) = 0 + j*fgap/4
        !            rb(m1,1) = 0 + i*fgap/4
        !            ncc0(i,1) = 0
        !            ncc0(i,2) = 0
        ...
        !    enddo       
        !enddo
        !do k = 1, big_tube_slices_num
        !    Hy_slice_high  = -7 + (k-1)*fgap/4 + small_tube_slices_num*fgap/4
        !    do j = -16, 16, 1
        !        do i = -16, 16, 1
        !            m1 = m1 + 1
        !            rb(m1,3) = Hy_slice_high
...
!                        
!                        
!                        if(abs(ri(3)) .gt. (regionH(3)-lenDpL)) goto 50
!                        
!                        do i = 1, ndpsb(idom)
!                            s = 0
!                            do k = 1, NDIM
