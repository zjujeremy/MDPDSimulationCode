...
!   if(runId .eq. 5 .and. stepCount .gt. stepEquil) then
    if(runId .eq. 5) then
        LEShiftX = ShearRate*region(3)*timeNow
        LEShiftV = ShearRate*region(3)
    endif
...
! ----- (DP--21/07/12)                                           -----
       if(stepCount .lt. stepEquil*0.5) then
          do n = nWallAtom + 1, nDpEnd
             do k = 1, NDIM
                ra(n,k)  = 0.
                rv(n,k)  = 0.
       ...
       if(stepCount .ge. stepEquil - 1 .and. stepCount .le. stepFzn) then
          do n = nWallAtom + 1, nDpEnd
             do k = 1, NDIM
                ra(n,k)  = 0.
                rv(n,k)  = 0.
...
!            write(111,'(''zone'')')
!            do i = 1, nPDP
!                k = nWallAtom + nPDP*(n-1) + i
!                write(111,'(1x,3f10.5)') r(k,1),r(k,2),r(k,3)
!            enddo
!        enddo
!    endif
...
!---------------------------------------------------------------------------
    ...
    elseif(icord .eq. 1) then
       ...
       enddo
!       do n = 1, nDp
!            do i = nWallAtom + nPDP*(n-1) + 1, nWallAtom + nPDP*n
!                do k = 1, NDIM
!                    ra(i,k) = frcTot(n,k) / nPDP
!                enddo
!            enddo
!       enddo
...
!    real*8 Vx1, Vx2, Vz1, Vz2
    ...
    open(31, file = './Paras_Config/elongCnf.dat', status='unknown')
