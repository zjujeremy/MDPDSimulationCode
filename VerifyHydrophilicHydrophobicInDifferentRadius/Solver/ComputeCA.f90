    ...
    real * 8 wr_temp,CA_Drop_r_Z(nDpEnd-nWallAtom),Centre_X,Centre_Y,gap_slice,cost_value,cost_value_temp,first_slice
            ...
            do  n = nWallAtom + 1, nDpEnd
                if(r(n,3) .ge. Drop_bottom/num_steps+(i-1)*gap_slice .and. r(n,3) .lt. Drop_bottom/num_steps+i*gap_slice) then
                     if (sqrt((r(n,1) - Centre_X)**2 + (r(n,2) - Centre_Y)**2) .le. first_slice) then
                         ...
                         n_cell = int((sqrt((r(n,1) - Centre_X)**2 + (r(n,2) - Centre_Y)**2)-first_slice)/deltaQ) + 2
                     endif
                     !n_cell = int((sqrt((r(n,1) - Centre_X)**2 + (r(n,2) - Centre_Y)**2))/deltaQ) + 1
    ...
    elseif(icord .eq. 2) then 
        ...
        do i = 1 , num_slice
            if(num_particle(i,1) .gt. 0) then
                do j = 1 , num_cell
                    if(j .eq. 1) then
                        density_value(i,j) = (1.0*num_particle(i,j)/num_steps)/(gap_slice*pi*first_slice**2)
                        !density_value(i,j) = (1.0*num_particle(i,j)/num_steps)/(gap_slice*pi*deltaQ**2)
                    else
                        density_value(i,j) = (1.0*num_particle(i,j)/num_steps)/(gap_slice*pi*(first_slice+deltaQ*(j-1))**2 - gap_slice*pi*(first_slice+deltaQ*(j-2))**2)
                        !density_value(i,j) = (1.0*num_particle(i,j)/num_steps)/(gap_slice*pi*(deltaQ*j)**2 - gap_slice*pi*(deltaQ*(j-1))**2)
                    endif  
...
!                        !each_slice_point_top(i,1) = deltaQ/20 + deltaQ*(j-1)
...
! 利用回归算法对液滴形状进行拟合----------x^2+(y-a)^2=b^2------------------------
        ! Contact Angles from buttom to top 
        cost_value = 0.0
        cost_temp = 0.0
        check_iter = 1
        m = 0
